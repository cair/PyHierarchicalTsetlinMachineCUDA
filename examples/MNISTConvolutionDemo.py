...
for i in range(X_train.shape[0]):
	...
	for x in range(1, patch_size):
		for y in range(1, patch_size):
			for z in range(patch_size-1):
				if z < x:
					X_train[i,:,patch_size*patch_size + (patch_size - 1) + z] = 1

				if z < y:
					X_train[i,:,patch_size*patch_size + z] = 1
...
for i in range(X_test.shape[0]):
	...
	for x in range(1, patch_size):
		for y in range(1, patch_size):
			for z in range(patch_size-1):
				if z < x:
					X_test[i,:,patch_size*patch_size + (patch_size - 1) + z] = 1

				if z < y:
					X_test[i,:,patch_size*patch_size + z] = 1
